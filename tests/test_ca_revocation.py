...
def test_revoke_unknown_certificate_errors(db: Session, secrets: SecretStore) -> None:
    ...
    assert db.get(CRLState, 1) is None


def test_revoke_without_a_ca_leaves_the_row_alone(db: Session, secrets: SecretStore) -> None:
    ...
    row = Certificate(
        ...
        sans_json='["DNS:orphan.lan"]',
        profile="server",
        not_before=_NOW.isoformat(),
...
def test_crl_number_monotonic(db: Session, secrets: SecretStore) -> None:
    ...
    # exactly one row: the CRL is a single current document, not a history
    assert db.query(CRLState).count() == 1
...
def test_crl_next_update_window(db: Session, secrets: SecretStore) -> None:
    ...
    fresh = current_crl(db, secrets, now=_NOW + timedelta(days=1))
    assert fresh.crl_number == published

    refreshed = current_crl(db, secrets, now=_NOW + CRL_VALIDITY + timedelta(seconds=1))
...
def test_crl_refreshed_before_it_expires(db: Session, secrets: SecretStore) -> None:
    ...
    comfortable = current_crl(db, secrets, now=_NOW + CRL_VALIDITY - timedelta(hours=2))
    ...
    refreshed = current_crl(db, secrets, now=due)
